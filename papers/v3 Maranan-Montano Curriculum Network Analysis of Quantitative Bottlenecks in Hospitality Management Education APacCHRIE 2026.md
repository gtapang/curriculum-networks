...
^1^ Department of Hotel, Restaurant and Institution Management, College
of Home Economics, University of the Philippines Diliman

^2^ National Institute of Physics, College of Science, University of the
Philippines Diliman

^3^ Data Science Program, College of Science, University of the
Philippines Diliman
...
in a rapidly growing bachelor's program for hospitality. The program has
...
prerequisite chain centered on subjects on calculus (CALC) and
statistics (STAT), with the operations management (ORM) subject
...
intended prerequisite tiers. CALC emerges as the dominant academic
...
failure-driven retakes. The dominant empirical hub by degree is PHYSED2,
a structural artifact of its four-time enrollment requirement. These
...
We discuss in this paper a bachelor's program in a local university for
hospitality management that has seen a period of unprecedented growth.
Between 2023 and 2025, new cohorts have grown to represent over 60% of
total enrollment, with section counts escalating from a single section
in 2021 to four sections by 2025. However, this growth has collided with
the structural rigidities of the 156-unit curriculum revised in 2021.
...
management (CUL) series requires successful completion of ORM
(Operations Management), a critical fifth-semester block that must be
taken in its entirety. ORM requires CALC and STAT. High failure rates in
...
BS HRIM, this has significant implications: early engagement predicts
course-level outcomes, and course-level outcomes in prerequisite-gating
courses predict degree-level trajectories. Identifying bottleneck
courses through network analysis is a critical first step in at-risk
...
for ORM, which is the gateway to the HRIM 150 series. Failure at any
node in this sequence blocks all downstream progression. Academic
...
*Figure 1. Structural Curriculum Network of the* *hospitality* *Program
(156 Units). Nodes represent individual courses; directed edges point
from prerequisite to dependent course. Node colour encodes Louvain
community membership (modularity Q = 0.674; red = quantitative chain
community, blue = HRIM foundation cluster, grey = GE and other
clusters). Node size is proportional to out-degree. Key bottleneck
courses (CALC,* *STAT, HRIM 120,* *OR) are labeled in the figure.*

**Hub Dominance**

Two major high-degree star subgraphs are evident: HRIM 120 (generally
taken during the first two years) and HRIM 150/HRIM 199 (junior
courses). HRIM 120 has a massive out-degree, serving as the prerequisite
for at least seven core courses. HRIM 150 and 199 act as gatekeepers to
senior courses with high betweenness centrality. Congestion at either
node constrains the entire graduation pipeline.
...
courses per stratum), while the later strata leading to the HRIM 150 and
...
Math 20, CALC, STAT, and ORM together with their downstream senior
electives, confirming this chain as a single structurally cohesive unit.
C2 (9 courses) captures the HRIM foundation cluster anchored by HRIM
120, encompassing all seven courses directly unlocked by HRIM 120, along
with its two prerequisite feeders (HRIM 110, HRIM 116). Smaller
...
prerequisite sub-chains converging at two chokepoints, HRIM 120 and OR,
before diverging again into the senior tier.
...
exceeds CALC's reach (17).

***Table 1. Structural Centrality Measures --- Key*** ***hospitality***
***Curriculum Nodes***
...
HRIM 120---converge and fan out into the HRIM 150 series, making it the
curriculum's structural chokepoint. Second, HRIM 120 has the highest
...
***Table 6. Course Completion Rates***
  ...
  **Core HRIM Course**    **Satisfied**           **%**
  ...
  HRIM 176--179, 200      25                      9.29%
  (group)                                         
  ----------------------- ----------------------- -----------------------

Almost all students have completed HRIM 110 (Macro Perspectives). Nearly
5% have yet to pass HRIM 116, indicating an early bottleneck. More
students have passed STAT than CALC. Low completion rates for
upper-level courses (HRIM 150+) are expected given the large incoming
cohorts.
...
***Table 7. Network Overview***
  ...
  **Metric**                          **Value**
  ...
  Highest in-degree node              PHYSED2 (in = 1,541)

  Highest out-degree node             PHYSED2 (out = 1,679)
...
prerequisites. HRIM 120 serves as the primary convergence sink,
...
![](media/image2.png){width="6.0in" height="4.673033683289589in"}

*Figure 2. Empirical Progression Network of* *hospitality* *Students (N
= 269, 2021--2026 Cohorts). Nodes represent courses as actually enrolled
in by students; directed edges connect sequentially enrolled courses
across semesters, with only transitions of five or more student
instances shown. Edge thickness proportional to transition frequency.
Node colour encodes Louvain community membership (Q = 0.231). Node size
proportional to weighted in-degree. Red rings indicate failure rate
above 5%. Self-loops on* *CALC,* *STAT, and* *PHYSED2 indicate
re-enrolment.*

![](media/image3.png){width="6.267716535433071in"
height="3.4166666666666665in"}

*Figure 3.* *hospitality* *Student Progression Flow (N = 269).
Three-tier curriculum funnel from enrollment through the quantitative
gate to upper-division HRIM coursework and near-completion. Grey = not
yet at quantitative gate (n ≈ 75); Red = retaking* *CALC or* *STAT (n ≈
23); Orange = pending next milestone; Green/Teal = milestone cleared.*
...
and 1.7: ORM (138 students, avg 1.69), HRIM 132 (148, avg 1.25), HRIM
140--144 (84--87 students, avg 1.36--1.52), and HE/FN/FS courses
...
through the quantitative gate (CALC + STAT), to OR, then to the HRIM
150+ series and near completion. It highlights three key flows: students
not yet at the quantitative gate; students stuck in quantitative retakes
(CALC/STAT); and students who have cleared ORM and are either pending
HRIM 150+ or already near/at completion. The width of each stream makes
clear that the largest losses and delays occur at the quantitative gate
and just before HRIM 150+, not in the upper-division HRIM core itself.
...
weighted degree = 408.4; max weighted in-degree = 1,541 (PHYSED2); max
weighted out-degree = 1,679 (PHYSED2).*
...
in-degree of 3 (OR) versus 55 (PHYSED2). Shortest path length decreases
...
***Table 9. Self-Loop Analysis***
  ...
  **Course**        **Retake Count**  **Avg Grade at    **Interpretation**
  ...
  PHYSED2           182               1.27              Structural: required
                                                        to take PE four
                                                        times
  ...
  CALC              71                4.94              Failure-driven: avg
                                                        grade ≈ 5.0,
                                                        mandatory retake

  PHYSED4           42                1.1               Structural:
                                                        student-athletes
                                                        take PHYSED4 four
                                                        times

  STAT              21                4.85              Failure-driven: avg
                                                        grade ≈ 5.0,
                                                        required retake

  HRIM 116          11                3.82              Mixed: near-failing
                                                        students repeating

  Kas 1             3                 4                 Failure-driven:
                                                        small number

  HRIM 110          1                 5                 Failure-driven:
                                                        single instance

  Philo 1           1                 5                 Failure-driven:
                                                        single instance

  Eng 1             1                 5                 Failure-driven:
                                                        single instance
...
The pattern is clear: CALC and STAT generate mandatory failure-retake
loops (average entry grade near 5.0), while PHYSED2 generates self-loops
because it must be taken 4 times.
...
prerequisite restrictions---specifically, failure to pass CALC on time.
...
early-semester transitions; a quantitative bottleneck (CALC and STAT)
that acts as a filter before OR; and a tightly convergent upper-division
HRIM core anchored by HRIM 120 (in-degree 1,106, 0.0% failure rate),
flowing into specialization courses and the terminal practicum cluster.
...
(Yang et al., 2024). CALC is an indirect prerequisite to the entire HRIM
150 series and all subsequent major courses. Any student who fails faces
two consequences: delay in that single course and effective lockout from
a large portion of their degree for at least one additional semester.
Yang et al. found introductory mathematics bottlenecks most severe at
the largest institutions. UP Diliman, as the flagship state university
of the Philippines, fits this pattern precisely, strengthening the
generalizability of the hospitality findings across different national
...
before progressing to the HRIM 150 series. Without this change, the CALC
self-loop continues indefinitely. All HRIM courses above HRIM 120 have
zero failures, suggesting that once students clear the quantitative
...
following the prescribed linear path, driven largely by the CALC/STAT
bottleneck forcing schedule reorganization.\
\
...
students who have not cleared CALC or STAT by end of their second
semester. These students are at elevated risk of multi-semester delays.
Early and proactive advising prevents the prerequisite trap from
compounding.
...
studies with other hospitality or hospitality management programs in the
